...
for file in only_files:
    ...
    if int(date.strftime("%Y")) >= 2020:     # only look at files from 2020 or newer
        if directory == "zip" or directory == "blend" or directory == "stl" or \
                directory == "pdf" or directory == 'gcode' or directory == "png" or \
                directory == "jpeg" or directory == "jpg":
            ...
            else:  # make a directory with the extension and move it to there
                mkdir(join(DOWNLOADS, file.split(".")[-1]))
                rename(join(DOWNLOADS, file), join(DOWNLOADS, file.split(".")[-1], file))
        elif directory == "dmg" or directory == 'pkg' or \
                directory == 'exe' or directory == '3mf':  # delete the installer packages
...
for folder in only_dirs:    # Move folders into a backup folder
    if 'backup' != folder.split("-")[0] or folder == 'Downloads':  # move folders to back-up folder
        try:
